...
@AdminArtists = React.createClass
	getInitialState: ->
	...
	updateArtist: (artist) ->
		...
		$.ajax
			...
			success: (data) =>
				@setState data: data
				artists = React.addons.update(@state.artists, { $splice: [[index, 1, { key: data.artist.id, id: data.artist.id, first_name: data.artist.first_name, last_name: data.artist.last_name, parlor_id: data.artist.parlor_id, image_file_name: data.artist.picture.image }]] })
				@replaceState artists: artists
	deleteArtist: (artist) ->
		index = @state.artists.indexOf artist
			...
			React.DOM.table
				...
				React.DOM.tbody null,
					for artist in @state.artists
						React.createElement AdminArtist, key: artist.id, id: artist.id, artist: artist, parlors: @state.parlors, handleDeleteArtist: @deleteArtist, handleEditArtist: @updateArtist
